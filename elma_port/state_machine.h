...
namespace elma {
    ...
    class StateMachine : public Process { 
        ...
        StateMachine(std::string name) : Process(name), _initial(NULL), _current(NULL), _propagate(false), _stateNames() {}

        //! Construct an unnamed StateMachine       
        StateMachine() : Process("unnamed state machine"), _initial(NULL), _current(NULL), _propagate(false), _stateNames() {}
        ...
        vector<string> get_state_names();
        vector<string> get_state_names_old();

        std::set<string> _stateNames;
