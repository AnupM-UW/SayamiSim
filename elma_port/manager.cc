...
namespace elma {
    ...
    Manager& Manager::watch(std::string event_name, std::function<void(Event&)> handler, int priority) {
        if (priority < 0) {
            throw std::range_error("Priority of handler has to be 0 or positive integer values");
        }
        event_handlers[event_name].push_back(make_tuple(priority, handler));
        return *this;
    }

    bool PriorityComparer(tuple<int, std::function<void(Event&)>> a, tuple<int, std::function<void(Event&)>> b) {
    ...
    Manager& Manager::emit(const Event& event) {
        ...
        string evtName = event.name();
        if ( event_handlers.find(evtName) != event_handlers.end() ) {
            // there is an event handler for this event
            vector< tuple <int, std::function<void(Event&)>>> handlers = event_handlers[evtName];
            for ( auto handlerRecord : event_handlers[evtName] ) {
                int priority = std::get<0>(handlerRecord);
                std::function<void(Event&)> handler = std::get<1>(handlerRecord);
                //// cout<<"Priority "<<priority<<" handler for "<<evtName<<endl;
            }

            std::sort(handlers.begin(), handlers.end(), PriorityComparer);
            for (auto hit = handlers.begin(); hit != handlers.end(); ++hit) {
                auto handler = std::get<1>(*hit);
                int pri = std::get<0>(*hit);
                //// cout<<pri<<" priority handler for "<<evtName<<" fired"<<endl;
                if ( e.propagate() ) {
                  handler(e);
                } else {
                    //// cout<<"Stopped propagation"<<endl;
                    break;
                }
            }
